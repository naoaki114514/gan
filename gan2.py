...
main_folder = "./D000013G0002_LeakyReLU_dropout01_soft"
...
def train(netD, netG, criterion, optimizerD, optimizerG, n_epoch, batch):
    ...
    for epoch in range(n_epoch):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            label = (0.7*torch.ones(batch_size, 1)).to(device)+ (1/200)*torch.randint(0,100,(batch_size,1)).float().to(device)#soft label 0.7 ~ 1.2
            ...
            fake = netG(noise)
            label = (1/1000)*torch.randint(0,300,(batch_size,1)).float().to(device)#soft label 0.0 ~ 0.3
